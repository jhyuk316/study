...
| 클래스   | 메서드               | 설명                              |
| -------- | -------------------- | --------------------------------- |
| Period   | long toTotalMonths() | 월단위로 변환해서 반환(일은 무시) |
| Duration | long toDays()        | 일단위로 변환해서 반환            |
| ''       | long toHours()       | 시단위로 변환해서 반환            |
| ''       | long toMinutes()     | 분단위로 변환해서 반환            |
| ''       | long toSeconds()     | 초단위로 변환해서 반환            |
| ''       | long toMillis()      | 밀리초단위로 변환해서 반환        |
| ''       | long toNanos()       | 나노초단위로 변환해서 반환        |
...
|   A    | 그 날의 밀리초(시각환산) |    number    | 1234                           |
|   N    | 그 날의 나노초           |    number    | 1234000000                     |
...
// 임의의 패턴
DataTimeFormatter pattern = DataTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
